...
public class MathUtil {
    ...
    /**
     * 获取目标值(目标值为空返回默认值，默认值为空返回目标值)
     * 
     * @param tagertValue 目标值
     * @param defaultValue 默认值
     * @return
     */
    public static BigDecimal getBDValue(BigDecimal tagertValue, BigDecimal defaultValue){
    	if(tagertValue == null){
     ...
     * @return
     */
    public static BigDecimal getBDValue(BigDecimal tagertValue){
    	return getBDValue(tagertValue, BigDecimal.ZERO);
    }
	 ...
	 * @return  
	 */
	public static BigDecimal BDadd(BigDecimal a, BigDecimal b, int scale, BigDecimal... params){
		if(a == null || b == null){
			throw new IllegalArgumentException("运算参数为空!");
		}
		BigDecimal targetValue = BDadd(a, b, -1);

		for(int i = 0; i < params.length; i++){
			targetValue = BDadd(targetValue, params[i], -1);
		}
		return round(scale, targetValue);
	}
}
