define(function(require, exports, module) {
	...
	var view = Backbone.View.extend({
		...
		queryRiskReport:function(){
			...
			$.ajax({
				type:'GET',
				url:$$ctx + "/monthlyBusiNotification/searchByCondition",//程序生成word
				data:{
					year:$('#year').val(),
					month:$('#month').val(),
				},
				success:function(result){
					if(result.data){
						//生成word另存为
						window.location.href=$$ctx + "/monthlyBusiNotification/download?outFileName="+result.data.outFileName;
					}else{
