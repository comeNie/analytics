...
public class AddReport {
	 ...
	 *
	 */
	public String addSpecToCM( CRNConnect connection ,ReportServiceSpecification reportSpec, String tempdirpath ,String reportName)
	throws java.rmi.RemoteException
	{
		...
		//String searchPath = "/content/folder[@name='abc']";
		try
		{
			connection.getReportService().add(new SearchPathSingleObject(tempdirpath), newReport, addReportOptions);
		}
		catch(java.rmi.RemoteException remoteEx)
		{
			remoteEx.printStackTrace();
		...
		return "Report " + reportName + " saved in " + tempdirpath + " in the content store.";
