...
public interface RoleMngDao extends PagingAndSortingRepository<Role, Integer>,
	...
	@Query("select new Role(r.id,r.name,r.status,r.ifTodoList,r.ifCheckRole) from Role r where r.id=?1")
	public Role getCheckRoleById(Integer id);
	...
	@Modifying
	@Query("update Role r set r.ifCheckRole=?1 where r.id=?2")
